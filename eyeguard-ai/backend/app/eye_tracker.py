...
class EyeTracker:
    ...
    def __init__(self, mode="health"):
        self.mode = mode  # "health" or "productivity"
    ...
    def process_frame(self, frame):
        ...
        # Mode-aware alert priority
        if self.mode == "productivity":
            # Productivity mode: drowsiness is top priority
            if metrics["drowsy"]:
            ...
            # Health mode: eye strain is top priority
            if metrics["bpm_drop_pct"] > 40 and bpm > 0:
                metrics["alert"] = "dry_eyes"
            elif metrics["too_close"]:
                metrics["alert"] = "too_close"
            elif metrics["drowsy"]:
